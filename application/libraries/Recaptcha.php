...
class ReCaptcha {
    ...
    function __construct() {
        ...
        if ($this->ci->config->item('recaptcha_secret_key') == NULL || $this->ci->config->item('recaptcha_secret_key') == "") {
            die("To use reCAPTCHA you must get an API key from <a href='"
                    . $this->signup_url . "'>" . $this->signup_url . "</a>");
        }
        if ($this->ci->config->item('recaptcha_site_key') == NULL || $this->ci->config->item('recaptcha_site_key') == "") {
            die("To use reCAPTCHA you must get an API key from <a href='"
                    . $this->signup_url . "'>" . $this->signup_url . "</a>");
        }
        $this->_secret = $this->ci->config->item('recaptcha_secret_key');
        $this->_sitekey = $this->ci->config->item('recaptcha_site_key');
        if ($this->ci->config->item('recaptcha_lang') == NULL || $this->ci->config->item('recaptcha_lang') == "") {
            $this->_lang = 'en';
        } else {
            $this->_lang = $this->ci->config->item('recaptcha_lang');
        }
    }
    ...
    public function verifyResponse($response, $remoteIp = NULL) {
        ...
        if (trim($answers['success']) == true) {
            ...
            $return = array(
                'success' => FALSE,
                'error_codes' => $answers['error_codes']
            );
        }
        return $return;
    }
